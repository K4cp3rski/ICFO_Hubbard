...
# =============================================================================
#     File showcasing a functioning example of 2-comp Hamiltonian use, with 
#     eigenenergies plotting. 
#
#
        ...
        for V_i in range(0, V_vec_aa.shape[0]):
            ...
            for delta_i in range(0, delta_vec.shape[0]):
                ...
                for evals_i in range(0, evals.shape[0]):
                    ...
                    e_i = evals[evals_i]
                    exp_val_H_1 = np.real(np.dot(np.conjugate(psi_H1).T, np.dot(H_1.toarray(), psi_H1)))
                    exp_val_H_2 = np.real(np.dot(np.conjugate(psi_H2).T, np.dot(H_2.toarray(), psi_H2)))
            ...
            for i in range(0, Evals1_matrix.shape[1]):
                ax[0].set_title(r"$\langle \lambda_1 | H_1 | \lambda_1 \rangle$", fontsize=18)
                ax[0].set_xlabel(r"$\Delta t$", fontsize=16)
                ax[0].set_ylabel(r"$E = \langle \lambda_1 | H_1 | \lambda_1 \rangle$", fontsize=20)
                ax[0].plot(delta_vec, Evals1_matrix[:, i])
                ax[1].set_title(r"$\langle \lambda_2 | H_2 | \lambda_2 \rangle $", fontsize=18)
                ax[1].plot(delta_vec, Evals2_matrix[:, i])
                ax[1].set_xlabel(r"$\Delta t$", fontsize=16)
                ax[1].set_ylabel(r"$E = \langle \lambda_2 | H_2 | \lambda_2 \rangle$", fontsize=20)
