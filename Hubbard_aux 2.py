...
def get_interactions_H(
    A, M, U, t_dict, statistic="b", component_no=1, component_count=1
):
    values = List()
...
def get_intercomp_interactions_H(
    A, M, V, t_dict, statistic=["b", "b"], component_count=2
):
    values = List()
        ...
        for ket in A:
            for i in range(0, M):
                ...
                if ket[comp_1 * ket_len + i] > 0 and ket[comp_2 * ket_len + i] > 0:
                    ...
                    val = (V) * factor_1 * factor_2 * factor_3 * factor_4
...
def get_Hamiltonian(
    ...
    if save_sparse:
        ...
        sc.sparse.save_npz(
            "{}_component/Hamiltonians/N={}_L={}/{}_Hamiltonian_N={}_L={}_U_J=".format(
                component_count,
                N,
                M,
                "".join(
    ...
    if plot_sparsity_pattern:
        ...
        if save_plot:
            ...
            if pbc == 1:
                ax.figure.savefig(
                    "{}_component/Plots/Hamiltonians/N={}_L={}/{}_Hamiltonian_sparse_N={}_L={}_U_J=".format(
                        component_count,
                        N,
                        M,
                        "".join(
    ...
    if plot_accurate:
        ...
        if save_plot:
            ...
            if pbc == 1:
                fig.savefig(
                    "{}_component/Plots/Hamiltonians/N={}_L={}/{}_Hamiltonian_N={}_L={}_U_J=".format(
                        component_count,
                        N,
                        M,
                        "".join(
    ...
    if component_no is None:
        ...
        if save_sparse:
            ...
            if not p.exists():
                p.mkdir(parents=True, exist_ok=True)
            sc.sparse.save_npz(
                "{}_component/Density_Matrices/N={}_L={}/{}_DM_N={}_L={}_U_J=".format(
                    component_count,
                    N,
                    M,
                    "".join(
        ...
        if save_plot:
            ...
            fig.savefig(
                "{}_component/Plots/Density_Matrices/N={}_L={}/{}_Hamiltonian_N={}_L={}_U_J=".format(
                    component_count,
                    N,
                    M,
                    "".join(
...
def _get_data_for_plots(M, N, J, U, pbc=0, k=0, basis_apriori=None, plot_type="sparse"):
    if basis_apriori is None:
    ...
    rho_k = get_density_matrix(
        N, M, Fock_Basis, evecs, pbc, U, J, save_sparse=True, save_plot=save_parameter
    )
...
def _generate_data(N, M, J, U_range=None, pbc=1, plot_type="sparse"):
    if U_range is None:
